function filterByTextParameters() {
	...
	for (let i = 0; i < pet_elements.length; ++i) {
		...
		for (let j = 0; j < parameters_ids.length; ++j) {
			...
			let pet_parameter = pet_element.querySelector(query_selector)
				.innerHTML;
			isVisible &=
				pet_parameter
					.toUpperCase()
...
function filterByCheckBoxes() {
	let sizes = [],
		genders = [];
	let sizeIDs = ["XS", "S", "M", "L", "XL"];

	sizeIDs.forEach((size) => {
		let element = document.getElementById("size_" + size);
		if (element.checked) sizes.push(size);
	});

	let sexIDs = { male: "M", female: "F" };

	Object.keys(sexIDs).forEach((key) => {
		let element = document.getElementById(key);
		if (element.checked) genders.push(sexIDs[key]);
	});
	...
	for (let i = 0; i < pet_elements.length; ++i) {
		...
		if (pet_element.style.display != "none") {
			if (sizes.length > 0) {
				let pet_parameter = pet_element.querySelector(
					query_selectors[0]
				).innerHTML;
