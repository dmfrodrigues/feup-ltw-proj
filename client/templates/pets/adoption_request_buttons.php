<?php $added_pets = getAddedPets($_SESSION['username']);
    ...
    <?php } else { 
                if (getAdoptionRequestOutcome($_SESSION['username'], $pet['id']) === 'pending') { ?>
                    <div id="remove-proposal"><a href="<?= PROTOCOL_SERVER_URL ?>/actions/remove_proposal.php?id=<?= $pet['id'] ?>" onclick="return confirm('Do you want to remove the adoption request?')"> <img src="resources/img/remove-proposal.svg" height="30px">Remove Proposal</a></span>
