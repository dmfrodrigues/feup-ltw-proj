    ...
    if (!checkIfAdopted($pet->getId())) {
        if (!in_array($pet, $added_pets)) { 
            if (!userRequestedPet($_SESSION['username'], $pet->getId())) { ?>
                <div id="propose"><a href="<?= PROTOCOL_SERVER_URL ?>/add_proposal.php?id=<?= $pet->getId() ?>"> <img src="resources/img/adopt-me.png" height="100px" width="100px"> </a></span>
    <?php } else { 
        ...
        <?php } else { ?>
        <div id="pet-already-adopted">The pet was already adopted by <a href="<?= PROTOCOL_SERVER_URL ?>/profile.php?username=<?=$userWhoAdoptedPet->getUsername()?>"><?=$userWhoAdoptedPet->getUsername()?></a> </div>
    <?php } }