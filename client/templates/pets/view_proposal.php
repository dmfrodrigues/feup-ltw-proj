    ...
    function drawPetProposal($reqId, $name, $adoptionMessage, $petId, $user, $outcome, $reqDate, $isMyPetProposal) { 
        if($outcome == 'pending') { ?>
            ...
            <div id="proposal-info">
                ...
                <?php if($isMyPetProposal) { ?>
                    <button onclick="location.href='<?= SERVER_URL ?>/action_change_adoptionRequest_outcome.php?requestId=<?=$reqId?>&username=<?=$_SESSION['username']?>&outcome=accepted&petId=<?=$petId?>'" id="acceptRequest">Accept Request</button>
                    <button onclick="location.href='requestAdoption.php?id=<?=$reqId?>'"id="answerRequest">Answer Request</button>
                    <button onclick="location.href='<?= SERVER_URL ?>/action_change_adoptionRequest_outcome.php?requestId=<?=$reqId?>&username=<?=$_SESSION['username']?>&outcome=rejected'" id="refuseRequest">Refuse Request</button>
                <?php } else { ?>
                    <button onclick="location.href='<?= SERVER_URL ?>/action_remove_proposal.php?id=<?=$petId?>'"id="cancelRequest">Cancel Request</button>
                <?php } ?>
